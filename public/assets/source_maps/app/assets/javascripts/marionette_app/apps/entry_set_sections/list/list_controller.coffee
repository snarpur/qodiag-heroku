@Qapp.module "EntrySetSectionsApp.List", (List, App, Backbone, Marionette, $, _) ->
  ...
  class List.Controller extends App.Controllers.Base
    ...
    showSidebarOnce:->
      console.log '@getEntryFieldsRegion()',@getEntryFieldsRegion()
      @getEntryFieldsRegion().on "show", (view) => 

        console.log "asf",arguments, view.model, region.$itemViewContainer
        @executeSidebar 
    ...
    executeFields:(options)->
      App.execute "show:settings:section:fields", 
        _.extend options, loaderRegion: @getSectionContentRegion(), region: @getEntryFieldsRegion()

      @showTitle(options.model)

      
    executeSidebar:(options) ->
      console.log "executeSidebar:(options) ->: ",options
